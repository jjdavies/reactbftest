...
const TeacherAndPuppet = React.memo((props) => {

    const [leave, setLeave] = useState(props.leave);
    const [point, setPoint] = useState(props.point);
    const [promptTeacher, setPromptTeacher] = useState("");

    const whatisthis = new uifx(whatisthisMP3)
    ...
    var lastFrameTime;
    var skeletons = {};
    var activeSkeleton = "teacherandpuppet";
    var startGame = false;

    var leaveEvent = new Event('leave')
    var talkEvent = new Event('talk')
    ...
    useEffect(()=>{
      ...
      setPoint(props.point || false)
      setPromptTeacher(props.promptTeacher)
      canvas = document.getElementById("teacherCanvas");
      if (props.leave === 2){
        canvas.dispatchEvent(leaveEvent)
      }
      console.log('props.promptTeacher', props.promptTeacher)
      if (props.promptTeacher){
        canvas.dispatchEvent(talkEvent)
      }
      // if (props.leave){
      //   console.log('trig')
      //     var state = skeletons[activeSkeleton].state;
    ...
    const init = () =>{
        ...
        assetManager.loadBinary("assets/teacherandpuppet.skel");
        assetManager.loadTextureAtlas("assets/teacherandpuppet.atlas");
    ...
    // Wait until the AssetManager has loaded all resources, then load the skeletons.
        if (assetManager.isLoadingComplete()) {
            skeletons["teacherandpuppet"] = loadSkeleton("teacherandpuppet", "enter", true);
            var state = skeletons[activeSkeleton].state;
            state.addAnimation(0, "idle", true, 0)
            if (point){
              state.addAnimation(1, "point", true, 0)
            }
            var stateData = state.data;
            ...
            canvas.addEventListener('leave', ()=>{
              ...
              state.setAnimation(0, animationName, false);
              // state.setAnimation(1, "point", false);
              // state.addAnimation(0, "idle", true, 0);
              console.log('leave', leave)
            }, false)

            canvas.addEventListener('talk', ()=>{
              var animationName = "talk";
              state.setAnimation(2, animationName, false);
              whatisthis.play();
              // state.setAnimation(1, "point", false);
              // state.addAnimation(0, "idle", true, 0);
              // console.log('leave', leave)
    ...
    const loadSkeleton = (name, initialAnimation, premultipliedAlpha, skin) => {
        ...
        var atlas = assetManager.get("assets/teacherandpuppet.atlas");
        ...
        var skeletonData = skeletonBinary.readSkeletonData(assetManager.get("assets/teacherandpuppet.skel"));
...
export default TeacherAndPuppet
